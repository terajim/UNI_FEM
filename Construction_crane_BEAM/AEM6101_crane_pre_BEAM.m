...
%ropes elements have Ad cross section
for m=size(elements,1)-2:size(elements,1)
    elements(m,4)=Ad;
end
...
k=1;
for i=1:122
    if Properties(i,2)==Ay || i==119 || i==120 || i==121 || i==122
        elements_beam(k,1)=k;
        elements_beam(k,2)=elements(i,2);
        elements_beam(k,3)=elements(i,3);
        elements_beam(k,4)=33; % column for the extra node "33"
        k=k+1;
    end
end
 
%NEW CROSS SECTION
%WE CHECK FROM TRUSS SCRIPT THE NUMBER OF FRAME'S ELEMENTS, THE NUMBER OF THE DIAGONALS ELEMENTS AND THEIR LENGTHS
A=0;
L_tot=0;
for i=1:size( elements,1)
    elements_length=norm( nodes(elements(i,3),2:4)- nodes( elements(i,2),2:4));
    A=A+elements_length*Properties(i,2);
    L_tot=L_tot+elements_length;
end
Abeam=A/L_tot;
    
for i=1:size(elements_beam,1)
    if i~=59 && i~=60 && i~=61 && i~=62
        elements_beam(i,5)=Abeam;
    else
        elements_beam(i,5)=Ad;
    end
end

%element properties

%need for an extra node in order to define beam's orientation
nodes(size(nodes,1)+1,1)=33;
nodes(size(nodes,1),2)=0;
nodes(size(nodes,1),3)=0;
nodes(size(nodes,1),4)=3*L;


%3 extra dof
nodes(:,5:7)=zeros(33,3);
nodes_beam=nodes;
...
q1=1;
 ...
 answer=listdlg('PromptString','Use Default boundary conditions?','SelectionMode','single','ListString',{'Yes','No'});
    ...
    str=ones(size(nodes_beam,1),1);
    for j=1:size(nodes_beam,1)
        str(j,1)=j;
        scatter3(nodes_beam(j,2),nodes_beam(j,3),nodes_beam(j,4),'red','filled')
        text(nodes_beam(j,2),nodes_beam(j,3),nodes_beam(j,4),{'node',num2str(str(j,1))})
        hold on
    end
    ...
    ylabel('y-axis [mm]')   
    zlabel('z-axis [mm]')
    for i=1:size(elements_beam,1)-4
        line(nodes_beam(elements_beam(i,2:3),2),nodes_beam(elements_beam(i,2:3),3),nodes_beam(elements_beam(i,2:3),4),'LineWidth',3)
    end 
    
    for i=size(elements_beam,1)-3:size(elements_beam,1)
        line(nodes_beam(elements_beam(i,2:3),2),nodes_beam(elements_beam(i,2:3),3),nodes_beam(elements_beam(i,2:3),4),'LineWidth',1)
    end    
    axis equal
 ...
 if answer==2
    ...
    while q1==1
         list=listdlg('PromptString','Please select a Boundary Condition Style','SelectionMode','single','ListString',{'x,y,z','x,y','x,z','y,z','x','y','z','èx,èy,èz','èx,èy','èx,èz','èyèz','èx','èy','èz'});
         hold on
         if list==1
             hold on
             shg
         ...
         elseif list==7
             ...
             camproj('orthographic')
             view([-8 -14 1])
             msgbox('Please select a node to lock its èx,èy,èz')
             waitforbuttonpress
             c_info = getCursorInfo(dcm_obj);
             set(c_info.Target,'MarkerEdgeColor','black')
             for i=1:size(nodes,1)
                if sum(c_info.Position==nodes(i,2:4))==3
                    bc_nodes(i,5:7)=1;
                end
             ...
             camproj('orthographic')
             view([-8 -14 1])
             msgbox('Please select a node to lock its èx,èy,')
             waitforbuttonpress
             c_info = getCursorInfo(dcm_obj);
             set(c_info.Target,'MarkerEdgeColor','magenta')
             for i=1:size(nodes,1)
                if sum(c_info.Position==nodes(i,2:4))==3
                    bc_nodes(i,5:6)=1;
                end
             ...
             camproj('orthographic')
             view([-8 -14 1])
             msgbox('Please select a node to lock its èx,èz')
             waitforbuttonpress
             c_info = getCursorInfo(dcm_obj);
             set(c_info.Target,'MarkerEdgeColor','magenta')
             for i=1:size(nodes,1)
             ...
             camproj('orthographic')
             view([-8 -14 1])
             msgbox('Please select a node to lock its èy èx ')
             waitforbuttonpress
             c_info = getCursorInfo(dcm_obj);
             set(c_info.Target,'MarkerEdgeColor','magenta')
             for i=1:size(nodes,1)
             ...
             camproj('orthographic')
             view([-8 -14 1])
             msgbox('Please select a node to lock its èx')
             waitforbuttonpress
             c_info = getCursorInfo(dcm_obj);
             set(c_info.Target,'MarkerEdgeColor','yellow')
             for i=1:size(nodes,1)
                if sum(c_info.Position==nodes(i,2:4))==3
                    bc_nodes(i,5)=1;
                end
             ...
             camproj('orthographic')
             view([-8 -14 1])
             msgbox('Please select a node to lock its èy')
             waitforbuttonpress
             c_info = getCursorInfo(dcm_obj);
             set(c_info.Target,'MarkerEdgeColor','yellow')
             for i=1:size(nodes,1)
             ...
             camproj('orthographic')
             view([-8 -14 1])
             msgbox('Please select a node to lock its èz')
             waitforbuttonpress
             c_info = getCursorInfo(dcm_obj);
             set(c_info.Target,'MarkerEdgeColor','yellow')
             for i=1:size(nodes,1)
    ...
    bc_nodes=zeros(size(nodes_beam,1),7); %boundary conditions matrix, 1 row for each node , 1-3 columns x y z , 4-6 columns èx èy èz 
    for i=1:size(nodes_beam,1)
        bc_nodes(i,1)=i;
    end
    hold on
    bc_nodes(1,2:4)=1;
    scatter3(nodes_beam(1,2),nodes_beam(1,3),nodes_beam(1,4),'magenta','filled')
    bc_nodes(2,2)=1;
    bc_nodes(2,4)=1;
    scatter3(nodes_beam(2,2),nodes_beam(2,3),nodes_beam(2,4),'yellow','filled')
    bc_nodes(size(nodes_beam,1)-3:size(nodes_beam,1)-1,2:4)=1;
    scatter3(nodes_beam(size(nodes_beam,1)-3:size(nodes_beam,1)-1,2),nodes_beam(size(nodes_beam,1)-3:size(nodes_beam,1)-1,3),nodes_beam(size(nodes_beam,1)-3:size(nodes_beam,1)-1,4),'magenta','filled')
 end
    ...
    if forceq==1
       ...
       for i=1:size(nodes,1)
         ...
         hold on
         force(29,4)=-20000;
         scatter3(nodes_beam(29,2),nodes_beam(29,3),nodes_beam(29,4),'green','filled')
         hold off
    end
    hold on
    ...
    legend([h1,h2,h3,h4,h5,h6],'0 fr degrees','1 fr degrees','2 fr degrees ','Force','Nodes','Elements')
    hold off
...
%create txt file for force applied
    ...
    for i=1:size(bc_nodes,1)
        fprintf(fID,'%d,    %d,      %d,      %d,    %d,    %d,     %d\r\n',bc_nodes(i,1),bc_nodes(i,2),bc_nodes(i,3),bc_nodes(i,4),bc_nodes(i,5),bc_nodes(i,6),bc_nodes(i,7));
    end
    fclose(fID);
    ...
    for i=1:size(elements_beam,1)
        fprintf(fID,'%d,    %d,     %d,     %d,    %d,    %d\r\n',elements_beam(i,1), elements_beam(i,2), elements_beam(i,3), elements_beam(i,4),elements_beam(i,5),Properties(i,3));
    end
    fclose(fID);
